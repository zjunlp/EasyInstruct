...
def generate_from_json_tar_gz(fname: str, target_file: str) -> Generator[Dict[str, Any], None, None]:
...
def add_space_biaodian(tokens, entity):
    '''
    ['Hello', ',', 'Nice', 'to', 'meet', 'you', '.', '你', '今天', '吃', '了', '饭', '吗']
    "Nicetomeet"
    '''
        ...
        if entity.startswith(tokens[i]):
            ...
            while start < len(tokens):
                if entity.startswith(tmp + tokens[start]):
                    tmp = tmp + tokens[start]
                    start += 1
                elif entity.startswith(tmp + tokens[start].rstrip(string.punctuation)):
                    tmp = tmp + tokens[start].rstrip(string.punctuation)
                    start += 1
                elif tokens[start] == " ":
                    start += 1
                else:
