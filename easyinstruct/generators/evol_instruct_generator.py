...
class EvolInstructGenerator(SelfInstructGenerator):
    ...
    def __init__(self, 
        ...
        super(EvolInstructGenerator, self).__init__(target_dir, seed_tasks_path, generated_instructions_path, generated_instances_path, num_instructions_to_generate, engine)
    ...
    def createConstraintsPrompt(self, instruction):
        prompt = base_depth_instruction.format("Please add one more constraints/requirements into #The Given Prompt#'")
        prompt += "#The Given Prompt#: \r\n {} \r\n".format(instruction)
        prompt += "#Rewritten Prompt#:\r\n"
        return prompt

    def createDeepenPrompt(self, instruction):
        prompt = base_depth_instruction.format("If #The Given Prompt# contains inquiries about certain issues, the depth and breadth of the inquiry can be increased.")
        prompt += "#The Given Prompt#: \r\n {} \r\n".format(instruction)
        prompt += "#Rewritten Prompt#:\r\n"
        return prompt

    def createConcretizingPrompt(self, instruction):
        prompt = base_depth_instruction.format("Please replace general concepts with more specific concepts.")
        prompt += "#The Given Prompt#: \r\n {} \r\n".format(instruction)
        prompt += "#Rewritten Prompt#:\r\n"
        return prompt
    
    def createReasoningPrompt(self, instruction):
        prompt = base_depth_instruction.format("If #The Given Prompt# can be solved with just a few simple thinking processes, you can rewrite it to explicitly request multiple-step reasoning.")
        prompt += "#The Given Prompt#: \r\n {} \r\n".format(instruction)
        prompt += "#Rewritten Prompt#:\r\n"
        return prompt
